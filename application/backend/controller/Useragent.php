...
class Useragent extends AuthController{
     ...
     * @return mixed|string
     */
    public function index($keyword = '', $page = 1,$startTime='',$endTime=''){
        try{
            $total_achievement = input('total_achievement');
            $list = UserAgentLogicQuery::getInstance()->agentList(['keyword'=>$keyword,'total_achievement'=>$total_achievement,'page'=>$page,'startTime'=>$startTime,'endTime'=>$endTime],$this->user_agent_model);
        }catch(\Exception $e){
